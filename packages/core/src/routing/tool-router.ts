...
import type { ConfirmationManager } from "../policy/confirmation-manager.js";
...
export class ToolRouter {
  ...
  async invoke(
    ...
    if (!tool) {
      await this.safeAudit({
        ...
        timestamp: started,
        runtimeToolId: "",
        mcpToolName: "mcpName" in request.target ? request.target.mcpName : request.target.runtimeId,
        sourceId: "",
        origin: "",
        decision: "deny",
        success: false,
    ...
    const source = this.options.sources.get(tool.identity.adapterId, tool.sourceId);
    if (!source) {
    ...
    if (tool.status !== "available" || source.state === "stale") {
      return errorResult("TOOL_UNAVAILABLE", "tool is not available", "not_executed");
    }
    if (tool.sourceGeneration !== source.generation) {
    ...
    if (decision.action === "confirm" && !this.options.confirmation.isChannelAvailable()) {
    ...
    try {
      const result = await this.queue.run(key, () =>
        this.dispatch(request, tool, source, options),
      );
      await this.safeAudit(
  ...
  private async dispatch(
    ...
    if (!latest || latest.generation !== source.generation) {
      throw new RuntimeError("TOOL_UNAVAILABLE", "generation changed before dispatch", "not_executed");
    }
  ...
  private safeOutcome(tool: RuntimeTool, started: boolean): "not_executed" | "executed_failed" | "unknown" {
